...
const Administration = () => {
  const [administrationNews, setAdministrationNews] = useState([]);
  const [otherNews, setOtherNews] = useState([]);
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);
  const BASE_URL = "http://localhost:3000";
  const currentCategory = "administration";
  ...
  const fetchAdministrationNews = async (
    category,
    page = 1,
    append = false
  ) => {
  ...
  useEffect(() => {
    fetchAdministrationNews("administration", 1);
  }, []);
  ...
  const handleLoadMore = (category) => {
    ...
    fetchAdministrationNews(category, nextPage, true);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-4">
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
              {administrationNews && administrationNews.length > 0 ? (
                administrationNews.map((item, idx) => (
                  <div
                    key={item._id || idx}
          ...
          {hasMore.administration && (
            <div className="col-span-full text-center mt-6">
              <button
                onClick={() => handleLoadMore("administration")}
